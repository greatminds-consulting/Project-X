                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="form-group" id="mm">
                            <label for="item_images" class="control-label" id="newcreate">Created By : <label id="created"></label></label>
                        ...
                        <div class="form-group">
                            <label for="stock" class="control-label"><?php echo _l('stock_in_hand'); ?></label>
                        ...
                        <div class="row">
                            <div class="form-group">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="text" class="form-control" name="margin" id="itemmargin" value="<?php echo $margin;?>" >
                                </div>
                                <div class="col-md-4">
...
//    $.post(url, data).done(function (response) {
    ...
    function init_item_js() {
        ...
        $("body").on('show.bs.modal', '#sales_item_modal', function (event) {
            ...
            $itemModal.find('input').not('input[type="hidden"]').not('[id="itemmargin"]').val('');
            ...
            $itemModal.find('#newcreate').addClass('hide');
            ...
            if (typeof (id) !== 'undefined') {
                ...
                requestGetJSON('invoice_items/get_item_by_id/' + id).done(function (response) {
                    ...
                    var alreadymargin="<?php echo $margin ?>";
                    if(response.margin!== null || response.margin!=='') {
                        var margin = parseInt(response.margin);
                    } else if(alreadymargin !==null || alreadymargin!=='') {
                        var margin = parseInt(alreadymargin);
                    } else {
                        var margin = 0;
                    }
                    var rate = parseInt(response.rate);
                    ...
                    if (!$.trim(response.item_image)){
                        $itemModal.find('#addeditem').attr('src',"" );
                    }
                    else{
                        $itemModal.find('#addeditem').attr('src',base_url +'/uploads/items/'+id +'/thumb_'+ response.item_image);
                    }
                        if(response.margin!== null || response.margin!=='') {
                        $itemModal.find('#itemmargin').val(response.margin);

                    }
                    else if(alreadymargin !==null || alreadymargin!=='')
                    {
                        $itemModal.find('#itemmargin').val(alreadymargin);
                    }
                    else
                    {
                        $('#itemmargin').val("");
                    }
                    $itemModal.find('input[name="stock"]').val(response.stock);
                    if(response.created_by== null ){
                        $('#created').html('Admin');
                    }else{
                        $itemModal.find('#created').html(response.created_by);}
                    ...
                    $itemModal.find('#newcreate').removeClass('hide');
                    validate_item_form();
                 });
