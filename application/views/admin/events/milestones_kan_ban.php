...
foreach ($milestones as $milestone) {
 ...
 $cpicker = '';
 if (has_permission('events', '', 'create') && $milestone['id'] != 0) {
 ...
 $total_pages = ceil($this->eventmanager_model->do_milestones_kanban_query($milestone['id'], $event_manager_id, 1, $milestonesTasksWhere, true)/get_option('tasks_kanban_limit'));

 $tasks = $this->eventmanager_model->do_milestones_kanban_query($milestone['id'], $event_manager_id, 1, $milestonesTasksWhere);
 ...
 <ul class="kan-ban-col milestone-column<?php if(!has_permission('events','','edit') || $milestone['id'] == 0){echo ' milestone-not-sortable';}; ?>" data-col-status-id="<?php echo $milestone['id']; ?>" data-total-pages="<?php echo $total_pages; ?>">
  ...
  <?php if ($milestone['id'] != 0 && (has_permission('tasks', '', 'create') || has_permission('events', '', 'create'))) { ?>
  ...
  <div class='text-center'><?php if (has_permission('tasks', '', 'create')) {
    ?><button type='button' return false;' class='btn btn-success btn-block mtop10 new-eventtask-to-milestone'>
...
<div class="kan-ban-content-wrapper">
  <div class="kan-ban-content">
   ...
   <li class="text-center not-sortable kanban-load-more" data-load-status="<?php echo $milestone['id']; ?>">
     <a href="#" class="btn btn-default btn-block<?php if ($total_pages <= 1) { echo ' disabled'; } ?>" data-page="1" onclick="kanban_load_more(<?php echo $milestone['id']; ?>,this,'eventmanager/milestones_kanban_load_more',320,360); return false;";>
