<?php if(!isset($discussion)){
    if($eventmanager->settings->open_discussions == 1){ ?>
<a href="#" onclick="new_discussion();return false;" class="btn btn-info mtop5"><?php echo _l('new_eventmanager_discussion'); ?></a>
<hr />
<!-- Miles Stones -->
<div class="modal fade" id="discussion" tabindex="-1" role="dialog">
    <div class="modal-dialog">
        <?php echo form_open(site_url('clients/eventmanager/'.$eventmanager->id),array('id'=>'discussion_form')); ?>
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">
                    <span class="edit-title"><?php echo _l('edit_discussion'); ?></span>
                    <span class="add-title"><?php echo _l('new_eventmanager_discussion'); ?></span>
                </h4>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <?php echo render_input('subject','eventmanager_discussion_subject'); ?>
                        <?php echo render_textarea('description','eventmanager_discussion_description'); ?>
                    </div>
                </div>
            </div>
...
<?php } ?>
    <table class="table dt-table" data-order-col="1" data-order-type="desc">
        <thead>
            <tr>
                <th><?php echo _l('eventmanager_discussion_subject'); ?></th>
        ...
        <tbody>
            ...
            <tr>
                <td><a href="<?php echo site_url('clients/eventmanager/'.$eventmanager->id.'?group='.$group.'&discussion_id='.$discussion['id']); ?>"><?php echo $discussion['subject']; ?></a></td>
                <td data-order="<?php echo $discussion['last_activity']; ?>">
                    <?php
                        if(!is_null($discussion['last_activity'])){
                            ...
                            $last_activity = _l('eventmanager_discussion_no_activity');
                        }
                        echo $last_activity;
                        ?>
                </td>
...
<p class="no-margin">
    <?php if($discussion->staff_id == 0){
        echo _l('eventmanager_discussion_posted_by',get_contact_full_name($discussion->contact_id));
        } else {
