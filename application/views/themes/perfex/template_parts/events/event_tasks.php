<?php if($eventmanager->settings->view_tasks == 1){?>
<!-- eventmanager Tasks -->
<?php if($eventmanager->settings->view_milestones == 1 && !isset($view_task)){ ?>
...
<div class="tasks-phases">
   <?php
      ...
      $milestones[] = array(
        ...
        'id'=>0,
        'total_logged_time'=>$this->eventmanager_model->calc_milestone_logged_time($eventmanager->id,0),
        'color'=>NULL,
        );
      $_milestones = $this->eventmanager_model->get_milestones($eventmanager->id);
   ...
   <div class="row">
      <?php foreach($milestones as $milestone){
         $tasks = $this->eventmanager_model->get_tasks($eventmanager->id,array('milestone'=>$milestone['id']));
      ...
      <div class="col-md-4 mtop25<?php if($milestone['id'] == 0 && count($tasks) == 0){echo ' hide'; } ?>">
         <div class="panel-heading <?php if($milestone_color != ''){echo 'color-not-auto-adjusted color-white ';} ?><?php if($milestone['id'] != 0){echo 'task-phase';}else{echo 'info-bg';} ?>"<?php echo $milestone_color; ?>>
            ...
            <?php if($eventmanager->settings->view_task_total_logged_time == 1){ ?>
         ...
         <div class="panel-body">
            ...
            <div class="media _task_wrapper<?php if((!empty($task['duedate']) && $task['duedate'] < date('Y-m-d')) && $task['status'] != 5){ echo ' overdue-task'; } ?>">
               <div class="media-body">
                  <a href="<?php echo site_url('clients/eventmanager/'.$eventmanager->id.'?group=event_tasks&taskid='.$task['id']); ?>" class="task_milestone pull-left<?php if($task['status'] == 5){echo ' line-throught text-muted';} ?>"><?php echo $task['name']; ?></a>
                  <?php if($eventmanager->settings->edit_tasks == 1 && $task['is_added_from_contact'] == 1 && $task['addedfrom'] == get_contact_user_id()){ ?>
                  <a href="<?php echo site_url('clients/eventmanager/'.$eventmanager->id.'?group=edit_task&taskid='.$task['id']); ?>" class="pull-right">
...
<div class="tasks-table <?php if($eventmanager->settings->view_milestones == 1){echo 'hide';} ?>">
      ...
      <table class="table dt-table table-tasks" data-order-col="0" data-order-type="asc">
         <thead>
            <tr>
               ...
               <?php if($eventmanager->settings->view_milestones == 1){ ?>
         ...
         <tbody>
            <?php
            foreach($eventmanager_tasks as $task){ ?>
            <tr class="<?php if($task['status'] != 5){echo 'task-unfinished-table';} ?>">
               <td class="hide" data-order="<?php echo $task['milestone_order']; ?>"></td>
               <td>
                  <?php if($eventmanager->settings->edit_tasks == 1 && $task['is_added_from_contact'] == 1 && $task['addedfrom'] == get_contact_user_id()){ ?>
                  <a href="<?php echo site_url('clients/eventmanager/'.$eventmanager->id.'?group=edit_task&taskid='.$task['id']); ?>"><i class="fa fa-pencil-square-o"></i></a>
                  <?php } ?>
                  <a href="<?php echo site_url('clients/eventmanager/'.$eventmanager->id.'?group=event_tasks&taskid='.$task['id']); ?>">
...
<?php } else {
   get_template_part('events/event_task');
}
}
?>
