<div id="task">
   <div class="row">
      <div class="col-md-12">
         <div class="row">
            <div class="col-md-9">
               <h3 class="no-mbot bold pull-left">
                  <?php if($eventmanager->settings->edit_tasks == 1 && $view_task->is_added_from_contact == 1 && $view_task->addedfrom == get_contact_user_id()){ ?>
                  <a href="<?php echo site_url('clients/eventmanager/'.$eventmanager->id.'?group=edit_task&taskid='.$view_task->id); ?>">
         ...
         <?php } ?>
         <?php if($eventmanager->settings->view_task_checklist_items == 1){ ?>
         ...
         <?php } ?>
         <?php if($eventmanager->settings->view_task_attachments == 1){ ?>
         <?php
                           ...
                           <div class="mbot10 pull-right task-attachment-user">
                              <?php
                                 if($attachment['staffid'] != 0){
                                   echo _l('eventmanager_file_uploaded_by') . ' ' . get_staff_full_name($attachment['staffid']);
                                 } else {
                                   echo _l('eventmanager_file_uploaded_by') . ' ' . get_contact_full_name($attachment['contact_id']);
                                 }
                                 ?>
                              <?php if(get_option('allow_contact_to_delete_files') == 1 && $attachment['contact_id'] == get_contact_user_id()){ ?>
                              <a href="<?php echo site_url('clients/delete_file/'.$attachment['id'].'/task?eventmanager_id='.$eventmanager->id); ?>" class="text-danger _delete pull-right"><i class="fa fa-remove"></i></a>
                              <?php } ?>
                           </div>
                           <?php
         ...
         <hr />
         <?php echo form_open_multipart(site_url('clients/eventmanager/'.$eventmanager->id),array('class'=>'dropzone mtop15','id'=>'task-file-upload')); ?>
         ...
         <?php } ?>
         <?php } ?>
         <?php if($eventmanager->settings->view_task_comments == 1){ ?>
         <hr />
         ...
         <div class="mbot10 mtop10" data-commentid="<?php echo $comment['id']; ?>" id="comment_<?php echo $comment['id']; ?>">
            ...
            <div class="media-body">
               ...
               <div class="comment-content" data-comment-content="<?php echo $comment['id'] ;?>">
                  <?php
                     if($comment['file_id'] != 0 && $eventmanager->settings->view_task_attachments == 1){
         ...
         <?php } }
            if($eventmanager->settings->comment_on_tasks == 1){
